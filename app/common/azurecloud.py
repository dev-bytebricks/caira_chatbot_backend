...
container_client_async: ContainerClientAsync = ContainerClientAsync.from_connection_string(
...
async def upload_file(username, file_content, file_name, content_type):
    ...
    try:
        await container_client_async.upload_blob(name=blob_name, 
                                           data=file_content, 
                                           overwrite=True, 
...
async def get_download_link(username, file_name):
    ...
    blob_client = container_client_async.get_blob_client(blob=blob_name)
